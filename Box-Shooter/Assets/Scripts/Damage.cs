...
public class Damage : MonoBehaviour {
	...
	{	
		if (damageOnCollision) {
			//if (this.tag == "PlayerBullet" && (collision.gameObject.tag == "Player" || collision.gameObject.tag == "Projectile" ))	// if the player got hit with it's own bullets, ignore it
			//	return;



			if (this.gameObject.GetComponent<Health> () != null) {	// if the hit object has the Health script on it, deal damage
				this.gameObject.GetComponent<Health> ().ApplyDamage (damageAmount);
